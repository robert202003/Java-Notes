...
## 28.ConcurrentLinkedQueue原理（难度星数：★★★★）

## 29.LinkedBlockingQueue原理（难度星数：★★★★）

## 30.ArrayBlockingQueue原理（难度星数：★★★★）

## 31.PriorityBlockingQueue原理（难度星数：★★★★）

## 32.DelayQueue原理（难度星数：★★★★）

## 33.SynchronousQueue（难度星数：★★★★）

## 34.LinkedTransferQueue（难度星数：★★★★）

## 35.LinkedBlockingDeque（难度星数：★★★★）

## 36.JAVA中父类与子类静态代码块、非静态代码块、构造函数的加载顺序（难度星数：★★）
...
## 41.反射中，Class.forName和classloader的区别（难度星数：★★）
java中class.forName()和classLoader都可用来对类进行加载。  class.forName()前者除了将类的.class文件加载到jvm中之外，还会对类进行解释，执行类中的static块。  
而classLoader只干一件事情，就是将.class文件加载到jvm中，不会执行static中的内容,只有在newInstance才会去执行static块。  
Class.forName(name, initialize, loader)带参函数也可控制是否加载static块。并且只有调用了newInstance()方法采用调用构造函数，创建类的对象。  

## 42.使用final关键字修饰一个变量时，是引用不能变，还是引用的对象不能变？（难度星数：★）
